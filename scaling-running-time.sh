...
# Plot
s=athena;            $SOURCE_DIR/scaling-running-time.py -s $s -l    -i $SOURCE_DIR/common.json -o $SOURCE_DIR/scaling-running-time-$s.pdf
s=athena-v2;         $SOURCE_DIR/scaling-running-time.py -s $s -l -y -i $SOURCE_DIR/common.json -o $SOURCE_DIR/scaling-running-time-$s.pdf
s=bigquery;          $SOURCE_DIR/scaling-running-time.py -s $s -l -y -i $SOURCE_DIR/common.json -o $SOURCE_DIR/scaling-running-time-$s.pdf
s=bigquery-external; $SOURCE_DIR/scaling-running-time.py -s $s -l -y -i $SOURCE_DIR/common.json -o $SOURCE_DIR/scaling-running-time-$s.pdf
